...
public:
  ...
  Matrix(int nr, int nc) {
    if (nr <= 0 || nc <= 0) {
      ...
      nrows = nr;
      ncols = nc;
      element = new complex<float>[nrows * ncols];
  ...
  complex<float> &operator()(const int &x, const int &y) const {
    return element[x * ncols + y];
  }
  // overload *= to calculate k*A
  void operator*=(const complex<float> &value) {
  ...
  friend Matrix blas_matmul(Matrix &A, Matrix &B) {
    ...
    complex<float> *matA = A.element;
    complex<float> *matB = B.element;
    complex<float> *matC = new complex<float>[m * n];
